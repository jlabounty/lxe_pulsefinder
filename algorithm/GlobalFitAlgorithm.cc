...
GlobalFitAlgorithm::ClusterCollection GlobalFitAlgorithm::Reconstruct(
  ...
  while (keepLooking) {
    ...
    std::transform(
      ...
      parameters.begin(),
      [](const auto &l_cluster) { return l_cluster.residualSeed; });
    ...
    std::size_t index = std::distance(window.begin(),
      ...
      [&](const auto &l_crystal) { return l_crystal.first < 0; }));
    ...
    if (metricSum < m_clusterThreshold) {
      ...
      std::transform(window.begin(), window.end(), crystals.begin(),
        [](const auto &l_crystal) { return l_crystal.first; });
      ...
      if (m_algorithmPrintLevel > 1) {
        std::cout << Form("Estimated peak time is %.3f c.t.",
          EstimateTime(residuals, p_indices, centralCrystal)) << std::endl;
      }
      ...
      do {
        ...
        for (const auto &cluster : clusterCollection.clusters) {
          ...
          if (m_algorithmPrintLevel > 2) {
            std::cout << Form("Adding seed %.3f c.t. with limits [%.3f, %.3f]",
              cluster.fitSeed, cluster.samples.front(), cluster.samples.back()) << std::endl;
          }
        ...
        if (m_algorithmPrintLevel > 1) {
          std::cout << Form("Fit chi-squared/ndf is %.3f/%i", clusterCollection.chiSquared,
            clusterCollection.degreesOfFreedom) << " at tweak " << std::showpos
            << clusterCollection.clusters.back().tweak << std::noshowpos << std::endl;

          std::cout << "Fit " << (clusterCollection.valid
            ? "converged" : "didn't converge") << std::endl;
        } // end status printout
        ...
        if (shareWindow     ||
          ...
          if (m_algorithmPrintLevel > 1) {
            std::cout << (shareWindow ?
              "Clusters found close in time in the identical window"    : (noiseIndex >= 0 ?
              "Cluster found with a template scale sum below threshold" :
              "Cluster found with a time at the parameter limit")) << std::endl;
        ...
        if (m_algorithmPrintLevel             > 2 &&
          ...
          std::cout << (clusterCollection.valid ? "Valid" : "Invalid")
            << " current state with chi-squared/ndf " << Form("%.3f/%i",
            clusterCollection.chiSquared, clusterCollection.degreesOfFreedom) << std::endl;
...
std::vector<GlobalFitAlgorithm::ClusterCollection> GlobalFitAlgorithm::Fit(
  ...
  do {
    ...
    for (std::size_t i = 0; i < parameters.size(); ++i) {
      ...
      auto &cluster = clusterCollections.at(catalog.at(i)).clusters.at(
        std::distance(std::find(catalog.begin(), catalog.end(), catalog.at(i)),
        catalog.begin() + i));
    ...
    for (std::size_t i = 0; i < clusterCollections.size(); ++i) {
      ...
      if (haveMerged) {
        ...
        const int index = std::distance(catalog.begin(), std::find(
          catalog.begin(), catalog.end(), i)) + mergedIndex;
        ...
        std::for_each(fixedClusters.begin(), fixedClusters.end(),
          [&](auto &l_cluster) { l_cluster -= (l_cluster > index ? 1 : 0); });
      ...
      if (!shareWindow) {
        noiseIndex = NoiseIndex(std::distance(catalog.begin(), std::find(
          catalog.begin(), catalog.end(), i)), clusterCollection.clusters, modelFunction);
      }
      ...
      if (shareWindow ||
        ...
        if (m_algorithmPrintLevel > 1) {
          std::cout << (shareWindow
            ...
            : "Cluster found with a template scale sum below threshold") << std::endl;
        } // end status printout
        ...
        modelFunction.RemoveCluster(std::distance(catalog.begin(), std::find(
          catalog.begin(), catalog.end(), i)) + index);
...
double GlobalFitAlgorithm::PseudotimeCorrection(
  ...
  const double psuedotime = (p_trace.at(peakIndex) == p_trace.at(peakIndex + 1))
    ? 1.0 : 2.0 / M_PI * std::atan(
    (p_trace.at(peakIndex) - p_trace.at(peakIndex - 1)) /
    (p_trace.at(peakIndex) - p_trace.at(peakIndex + 1)));
...
std::tuple<bool, bool, int> GlobalFitAlgorithm::MergeClusters(
      ...
      if (separation <= m_artificialDeadTime) {
        ...
        if (m_algorithmPrintLevel > 1) {
          std::cout << Form("Clusters # %lu and %lu are separated by %.5f c.t.",
            p_offset + i, p_offset + j, separation) << std::endl;
        }
  ...
  if (shareCrystals && !shareWindow) {
    ...
    p_clusters.at(closeClusters.first).centers.insert(
      p_clusters.at(closeClusters.first ).centers.end(),
      p_clusters.at(closeClusters.second).centers.begin(),
      p_clusters.at(closeClusters.second).centers.end());
...
int GlobalFitAlgorithm::NoiseIndex(
  ...
  for (std::size_t i = 0; i < p_clusters.size(); ++i) {
    ...
    if (scaleSum < sumThreshold && (
        noiseIndex == -1 || scaleSum < noiseTotal)) {
  ...
  if (noiseIndex >= 0 &&
    ...
    std::cout << Form("Cluster # %i has a template scale sum of %.3f ADU",
      p_offset + noiseIndex, std::accumulate(
      p_clusters.at(noiseIndex).scales.begin(),
      p_clusters.at(noiseIndex).scales.end(), 0.0)) << std::endl;
  }
